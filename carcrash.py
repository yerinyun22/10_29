...
Streamlit app (완성형)
기능 요약:
1) 사고 위치 마커 표시 (심각도 색상 구분)
2) 사고다발지 히트맵 / Hexagon(클러스터) 레이어
3) 날짜/연도/유형/원인 등 필터(해당 컬럼이 있으면 활성)
4) 검색(지역명/위치명) 및 클릭 시 툴팁(팝업) 표시 (pydeck tooltip)
5) 안전 경로(근사) : 출발지/도착지 선택 -> 여러 후보 궤적 생성 -> 각 후보의 "근접 위험도" 합계 계산 -> 최소 위험 후보 표시
6) 위험 구간 경고(근거리 필터) : 현재(혹은 입력 좌표) 주변 위험 반경 알림(데이터 기반)
7) 대한민국(서울 포함) 범위에서 작동 — CSV에 위도/경도 있어야 함

제약: 실제 도로 네트워크 기반 내비게이션은 외부 라우팅 API 필요(여기선 근사 방식).
"""
...
st.caption("제작: 예린 버전 · 데이터: 한국도로교통공단 · 구현: ChatGPT (근사/연구용)")
